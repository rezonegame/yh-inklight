...
成员清单
types.ts: sidecar JSON、注释、便签、代码选区标记、EPUB 阅读排版 profile（含本机字体覆盖字段）、设置和索引的类型真相源。
annotationStore.ts: .obsidian-annotations 持久化入口，负责文件 JSON、index、导出与迁移；资料库只读已有 sidecar、避免未读书籍哈希；写入前备份并严格回读校验。

...
### v0.18.1
- 移除书签模块（书签功能作用有限）：删除 Mod+Shift+B 命令、侧栏书签区、EPUB 书签按钮与列表、相关样式。数据层 ReadingBookmark 类型与 store.addBookmark/removeBookmark 保留（兼容旧 sidecar 中的书签数据，只是不再有 UI 入口）。

### v0.18.0
三格式一致性大版本：多个新功能，MD/PDF/EPUB 统一。

**摘录导出模板系统**：导出格式从硬编码改为 `{{text}}`/`{{color}}`/`{{pageNumber}}`/`{{cfi}}` 等变量模板（设置页可自定义，留空用内置默认）。默认模板与 v0.17 逐字节兼容。补 PdfGotoHandler 让 PDF 摘录回链真正可点击跳页。

**侧栏按位置过滤**：侧栏新增「仅显示当前位置」toggle，开启后只显示当前页（PDF）/当前章节（EPUB）的批注，翻页自动刷新。

**侧栏↔源文档 hover 联动**：悬停侧栏批注卡片 → 源文档对应高亮闪烁；悬停源文档高亮 → 侧栏对应卡片滚动高亮。三格式统一（补齐 EPUB foliate 高亮的 data-yh-id）。

**PDF 目录修复 + 文本锚点升级 + PDF 深链**：
- 修复 PDF 目录 named destination 解析 bug（此前条目静默消失）
- PDF 目录升级为可点击 Modal 列表（递归树 + 缩进，点击跳页闪烁）
- PDF 高亮新增 textLayer 文本锚点（抗旋转/重排），旧数据自动回落 rect
- PDF 摘录回链支持跳页 + 高亮区域闪烁定位

**UI 统一提升**：交互态完整（hover/active/focus-visible 四态）、GPU 友好过渡动画、统一 CSS 变量。
